select  dc.acq_ref_number as ARN,     
        ...
        coalesce(DB.rpr.get_tag_value(dc2.add_info,'RRN_IPSP'), (select rp.RRN_IPSP from report rp where DB.rpr.get_tag_value(dc2.add_info,'ID_ACCB_EXT_DATA')=rp.id)) as RRN_IPSP,
        ...
        (select currcode.name from DB.currency currcode where currcode.amnd_state='A' and currcode.CODE=dc2.trans_curr) as Cur, /*обращаемся к таблице с кодами валют и заменяем значение кода на наименование валюты.*/
        dc.settl_amount as Cbk_Amount,
        (select currcode.name from DB.currency currcode where currcode.amnd_state='A' and currcode.CODE=dc.settl_curr)  as Cbk_curr, /*обращаемся к таблице с кодами валют и заменяем значение кода на наименование валюты.*/
        ...
        (select trntype.name  trntype from DB.trans_type trntype where trntype.amnd_state='A' and trntype.id=dc.trans_type) as Trans_type, /*обращаемся к таблицн с транс типом и меняем цифровой код на наименование.*/
        ...
        (select mesch.name from DB.mess_channel mesch where mesch.amnd_state='A' and mesch.code=dc.source_channel) as System /* из таблицы каналов берем значение кода и вместо буквы выводим название канала.*/    
from
        DB.table dc,
        DB.table dc2
where
